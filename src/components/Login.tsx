...
export const Login: React.FC<LoginProps> = ({ onLoginSuccess }) => {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const matchedBuilding = buildings.find(b => b.presidentEmail.toLowerCase().trim() === cleanEmail);
      if (matchedBuilding) {
        setActiveBuilding(matchedBuilding);
      } else {
        const currentB = buildings.find(b => b.id === selectedBuildingId);
        if (currentB) setActiveBuilding(currentB);
      }
      ...
      if (data.success) {
        ...
        const user = {
          ...
          buildingId: activeB.id,
          buildingName: activeB.name,
        };
      ...
      <div className="w-full max-w-lg bg-white dark:bg-[#111a2e] rounded-3xl p-6 sm:p-8 border border-slate-200/80 dark:border-slate-800 shadow-2xl flex flex-col transition-all">
        ...
        {topTab === 'SIGN_IN' && (
          <div className="space-y-4">
            {buildings.length === 0 ? (
              <div className="text-center py-7 px-4 bg-slate-50 dark:bg-slate-800/40 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700 space-y-3">
                <div className="w-12 h-12 mx-auto bg-emerald-100 dark:bg-emerald-950/60 rounded-2xl flex items-center justify-center shadow-inner">
                ...
                <div>
                  <h3 className="text-sm font-black text-slate-800 dark:text-white mb-1">
                    لا توجد اتحادات ملاك مسجلة بعد
                  </h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400 max-w-xs mx-auto leading-relaxed font-medium">
                    لبدء استخدام المنظومة، يُرجى الضغط على زر «تسجيل عمارة جديدة» لإنشاء اتحادك الخاص وتعيين بيانات رئيس الاتحاد.
                  </p>
                </div>
                <button
                  type="button"
                  ...
                  className="py-2.5 px-5 bg-emerald-700 hover:bg-emerald-800 text-white rounded-xl text-xs font-black transition cursor-pointer shadow-md shadow-emerald-700/20 inline-flex items-center gap-2"
                >
                  <Sparkles className="w-4 h-4 text-emerald-200" />
                  <span>تسجيل اتحاد وعمارة جديدة الآن ✨</span>
                </button>
              </div>
            ) : (
              <>
                {/* Building Switcher Dropdown & Management */}
                {buildings.length > 0 && (
