...
export const Login: React.FC<LoginProps> = ({ onLoginSuccess }) => {
  ...
  const handleEnterDemoMode = async () => {
    ...
    try {
      // Avoid registering multiple demo unions. Check if one already exists in the local buildings list.
      const existingDemo = buildings.find(b => b.name === "اتحاد تجريبي (الوضع التجريبي)");
      if (existingDemo) {
        setSuccessMessage("تم العثور على اتحاد تجريبي نشط مسبقاً، جاري الانتقال للوحة التحكم...");
        setTimeout(() => {
          onLoginSuccess({
            userId: 'president-demo',
            email: 'demo@pyramidsview.com',
            role: 'ADMIN',
            name: 'مدير النظام التجريبي',
            flatNumber: 'ADMIN',
            phone: '01000000000',
            buildingId: existingDemo.id,
            buildingCode: existingDemo.code,
            buildingName: existingDemo.name,
          }, 'local-token');
        }, 500);
        return;
      }
      ...
      <div className="w-full max-w-lg bg-white dark:bg-[#111a2e] rounded-3xl p-6 sm:p-8 border border-slate-200/80 dark:border-slate-800 shadow-2xl flex flex-col transition-all">
        ...
        {topTab === 'SIGN_IN' && (
          <div className="space-y-4">
            ...
            {portalMode === 'PRESIDENT' && (
              <div className="space-y-4">
                {/* Primary Google or Demo Sign-In Card */}
                {(currentActiveBuilding?.name?.includes('تجريبي') || currentActiveBuilding?.id?.includes('demo')) ? (
                  <div className="p-4 bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-950/40 dark:to-orange-950/40 border-2 border-amber-200 dark:border-amber-800/70 rounded-2xl text-center space-y-3 shadow-xs">
                    <div className="flex items-center justify-center gap-1.5 text-amber-950 dark:text-amber-100 font-extrabold text-xs">
                      <Sparkles className="w-4 h-4 text-amber-500 shrink-0 animate-pulse" />
                      <span>الدخول المباشر والسريع للاتحاد التجريبي 🚀</span>
                    </div>
                    <p className="text-[11px] text-slate-600 dark:text-slate-300 font-bold leading-relaxed">
                      لقد اخترت الاتحاد التجريبي النشط. اضغط أدناه للدخول الفوري للاتحاد واستعراض كافة المزايا دون الحاجة لحساب جوجل أو كلمة مرور:
                    </p>
                    <button
                      type="button"
                      onClick={handleEnterDemoMode}
                      disabled={loading}
                      className="w-full py-3 px-4 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white rounded-xl text-xs sm:text-sm font-black transition flex items-center justify-center gap-2.5 shadow-md hover:shadow-lg cursor-pointer active:scale-[0.99]"
                    >
                      <LogIn className="w-5 h-5" />
                      <span>دخول سريع للاتحاد التجريبي (الوضع التجريبي)</span>
                    </button>
                  </div>
                ) : (
